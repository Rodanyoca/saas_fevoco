...
export function MedecinFormDialog({ medecin, sexes, onSaved }: {
  ...
  const [form, setForm] = useState({
    ...
    sexe: sexes.some((option) => option.nom === medecin?.sexe) ? medecin?.sexe ?? "" : "",
    dateDeNaissance: medecin?.dateDeNaissance ?? "", nationalite: medecin?.nationalite ?? "",
    telephone: medecin?.telephone ?? "", email: medecin?.email ?? "",
    adresse: medecin?.adresse ?? "", specialite: medecin?.specialite ?? "",
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild><Button variant={editing ? "outline" : "default"}>{editing ? "Modifier le médecin" : "Créer un médecin"}</Button></DialogTrigger>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-3xl">
        <DialogHeader><DialogTitle>{editing ? "Modifier le médecin" : "Créer un médecin"}</DialogTitle><DialogDescription>Le nom et le sexe sont obligatoires. Les avatars sont gérés séparément.</DialogDescription></DialogHeader>
        <form onSubmit={submit} className="space-y-6">
          ...
          <DialogFooter><Button type="button" variant="outline" disabled={pending} onClick={() => setOpen(false)}>Annuler</Button><Button type="submit" disabled={pending}>{pending ? "Enregistrement..." : editing ? "Enregistrer" : "Créer le médecin"}</Button></DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
  )
