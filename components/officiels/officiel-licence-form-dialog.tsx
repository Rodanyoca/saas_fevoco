...
export function OfficielLicenceFormDialog({ officiel, hasAffiliation, onSaved }: {
  officiel: Officiel
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    setError("")
    try {
      const response = await fetch(`/api/officiels/${encodeURIComponent(officiel.idOfficiel)}/licences`, {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader><DialogTitle>Créer une licence pour {officiel.nomComplet}</DialogTitle><DialogDescription>{hasAffiliation ? "La licence sera rattachée à l’officiel affilié." : "Créez d’abord une affiliation pour cet officiel."}</DialogDescription></DialogHeader>
