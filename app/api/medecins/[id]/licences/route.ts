...
import { createMedecinLicence } from "@/lib/medecin-licence-mutations"
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const result = await createMedecinLicence(decodeURIComponent(id), await request.json())
    revalidatePath("/medecins")
    return NextResponse.json({ message: "La licence du médecin a été créée avec succès.", ...result }, { status: 201 })
  } catch (error) {
