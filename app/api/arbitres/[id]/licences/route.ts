...
import { createArbitreLicence } from "@/lib/arbitre-licence-mutations"
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const result = await createArbitreLicence(decodeURIComponent(id), await request.json())
    revalidatePath("/arbitres")
    return NextResponse.json({ message: "La licence de l’arbitre a été créée avec succès.", ...result }, { status: 201 })
  } catch (error) {
